...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  const navItems = [
    ...
    { id: 'banners', label: 'Bannières', icon: ImageIcon },
    {
      ...
      badgeColor: 'bg-[#D6AE62] text-[#0B0B0B]',
    },
    {
      id: 'reviews',
      ...
      badgeColor: 'bg-amber-400 text-[#0B0B0B] font-bold',
    },
  ...
  return (
    <div className="min-h-screen bg-[#0B0B0B] text-[#F8F5EF] flex">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col justify-between w-64 bg-[#121212] border-r border-[#D6AE62]/20 shrink-0">
        <div>
          {/* Brand header */}
          <div className="p-6 border-b border-white/5">
            <Logo size="sm" />
            <div className="mt-3 flex items-center justify-between">
              <span className="text-[10px] font-mono uppercase tracking-wider text-[#D6AE62] bg-[#0B0B0B] px-2 py-0.5 border border-[#D6AE62]/30">
                {user.role}
              </span>
              <button
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3.5 py-2.5 text-xs font-medium transition-colors ${
                    isActive
                      ? 'bg-[#D6AE62] text-[#0B0B0B] font-bold shadow-sm'
                      : 'text-[#E5E5E5] hover:bg-[#1A1A1A] hover:text-white'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <Icon className={`w-4 h-4 ${isActive ? 'text-[#0B0B0B]' : 'text-[#D6AE62]'}`} />
                  ...
                  {item.badge && (
                    <span
                      className={`text-[10px] font-mono font-bold px-1.5 py-0.2 rounded-sm ${
                        item.badgeColor || (isActive ? 'bg-[#0B0B0B] text-[#D6AE62]' : 'bg-[#1E1E1E] text-[#D6AE62]')
                      }`}
                    >
                      {item.badge}
                    </span>
        ...
        <div className="p-4 border-t border-white/5 bg-[#0D0D0D]">
          <div className="flex items-center gap-3 mb-3 px-2">
            <div className="w-8 h-8 rounded-full bg-[#151515] border border-[#D6AE62]/40 flex items-center justify-center text-[#D6AE62]">
              <ShieldCheck className="w-4 h-4" />
            </div>
            <div className="overflow-hidden">
              <p className="text-xs font-semibold text-white truncate">{user.name}</p>
              <p className="text-[10px] font-mono text-[#A6A6A6] truncate">{user.email}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile Top Header */}
        <header className="lg:hidden bg-[#121212] border-b border-[#D6AE62]/20 p-4 flex items-center justify-between sticky top-0 z-30">
          <Logo size="sm" />

          <div className="flex items-center gap-2">
            <button
              onClick={onBackToStore}
              className="text-xs text-[#A6A6A6] hover:text-white p-2"
              title="Voir le site"
            >
            ...
            <button
              ...
              className="p-2 text-[#D6AE62] border border-[#D6AE62]/30"
            >
        ...
        {mobileNavOpen && (
          <div className="lg:hidden fixed inset-0 top-[61px] bg-[#0B0B0B]/95 backdrop-blur-md z-40 p-4 overflow-y-auto">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3 text-xs font-medium ${
                      isActive ? 'bg-[#D6AE62] text-[#0B0B0B] font-bold' : 'text-white'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                    ...
                    {item.badge && (
                      <span className="text-[10px] font-mono px-2 py-0.5 bg-[#0B0B0B] text-[#D6AE62]">
                        {item.badge}
                      </span>
                    )}
                  </button>
