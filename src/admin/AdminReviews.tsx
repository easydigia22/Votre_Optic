  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-[#151515] border border-white/5 p-4">
          <span className="text-[11px] uppercase tracking-wider text-[#A6A6A6]">Total Avis</span>
          <p className="font-mono text-2xl font-bold text-white mt-1">{totalCount}</p>
        </div>

        <div className="bg-[#151515] border border-amber-500/30 p-4">
        ...
        <div className="bg-[#151515] border border-emerald-500/30 p-4">
        ...
        <div className="bg-[#151515] border border-[#D6AE62]/30 p-4">
          <span className="text-[11px] uppercase tracking-wider text-[#E8C987]">Note Moyenne</span>
          <div className="flex items-baseline gap-1.5 mt-1">
            <p className="font-mono text-2xl font-bold text-[#E8C987]">{globalAverage}</p>
            <span className="text-xs text-[#A6A6A6]">/ 5.0</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#151515] border border-white/5 p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <span className="text-xs text-[#A6A6A6]">Statut :</span>
          <select
            value={filterStatus}
      ...
      <div className="space-y-4">
        {filteredReviews.length > 0 ? (
          filteredReviews.map((rev) => {
            ...
            return (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2.5">
                      ...
                      {rev.city && (
                        <span className="text-xs text-[#A6A6A6]">({rev.city})</span>
                      )}
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Glasses className="w-3.5 h-3.5 text-[#D6AE62]" />
                      <span className="text-xs text-white font-semibold">{rev.productName}</span>
                      <span className="font-mono text-[11px] text-[#A6A6A6]">[{rev.productRef}]</span>
                    </div>
                  </div>
                ...
                {/* Comment Content */}
                <div className="bg-[#0B0B0B] border border-white/5 p-4 my-3 text-xs">
                  {rev.title && (
                    <h5 className="font-semibold text-white mb-1">{rev.title}</h5>
                  )}
                  <p className="text-[#CCCCCC] leading-relaxed">{rev.comment}</p>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {rev.status !== 'rejected' && (
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#1C1C1C] hover:bg-red-950/60 border border-white/10 hover:border-red-800 text-neutral-300 hover:text-red-300 text-xs flex items-center gap-1.5 transition-colors"
                      >
          ...
          <div className="bg-[#151515] p-12 text-center text-[#A6A6A6] border border-white/5 text-xs">
