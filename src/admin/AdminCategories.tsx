    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.map((cat) => (
          <div
            key={cat.id}
            className="bg-[#1B1A15] border border-white/5 overflow-hidden flex flex-col justify-between"
          >
            <div>
              <div className="relative h-40 bg-[#11110F] overflow-hidden">
                <img
                  src={cat.image}
                  alt={cat.name}
            ...
            <div className="p-4 border-t border-white/5 bg-[#15140F] flex items-center justify-between">
              <div className="flex items-center gap-1">
                <button
                  onClick={() => handleMoveOrder(cat, -1)}
                  title="Monter"
                  className="p-1 text-[#9F9A8E] hover:text-white border border-white/10"
                >
          ...
          <div className="w-full max-w-lg bg-[#1B1A15] border border-[#C6A53A]/40 p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[#9F9A8E]">Ordre :</span>
                  <input
                    type="number"
                    min="1"
                    value={form.order}
                    onChange={(e) => setForm((p) => ({ ...p, order: Number(e.target.value) }))}
                    className="w-16 bg-[#11110F] border border-white/10 text-center text-white p-1"
                  />
                </div>
              </div>
